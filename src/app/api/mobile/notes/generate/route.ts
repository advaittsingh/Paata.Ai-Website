...
export async function POST(request: NextRequest) {
  try {
    ...
    const { topic, conversationHistory, sourceType, format, autoSave, category, tags } = body;
    ...
    try {
      ...
      if (autoSave && generatedNotes.title && generatedNotes.content) {
        try {
          savedNote = await PrismaDatabase.createNote({
          ...
          // Continue and return generated notes even if save fails
        }
      }
    ...
    console.error('Mobile notes generation API error:', error);
    return NextResponse.json(
      { 
        success: false,
